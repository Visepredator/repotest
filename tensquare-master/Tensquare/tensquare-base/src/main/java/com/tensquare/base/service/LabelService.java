...
public class LabelService {
     ...
     * @return
     */
    public Label finaById(String id){
    ...
    public List<Label> findSearch(Label label) {
      ...
      return   labelDao.findAll(new Specification<Label>() {
           ...
           * @param criteriaQuery  封装的都是查询的关键字，比如group by order by等
           * @param criteriaBuilder 用了封装条件对象的,如果直接返回null，表示不需要任何条件
           * @return
           */
            @Override
            public Predicate toPredicate(Root<Label> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
              ...
              predicates = list.toArray(predicates);
                 return criteriaBuilder.and(predicates);//where libername like "%小明%" and state = "1"
            }
        });
